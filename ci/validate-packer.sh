...
image_id=$(openstack image show -c name -c id -f table  "${PACKER_IMAGE_NAME}" || echo -n "") && test -z "${image_id}" && echo "There is no existing image called '${image_name}' (ok!)" || (echo "An image named '${image_name}' already exists, refusing to continue!" && exit 1)
