...
if [ "${CI_BUILD_STAGE}" == "deploy" ]; then
    echo "$0 setting OS deploy variables"
    ...
    echo "Using OS_AUTH_URL=[${OS_AUTH_URL}] OS_USERNAME=[${OS_USERNAME}]"
else
    echo "$0 setting OS packer variables"
    ...
    echo "Using OS_AUTH_URL=[${OS_AUTH_URL}] OS_USERNAME=[${OS_USERNAME}] OS_TENANT_NAME=[${OS_TENANT_NAME}] OS_NETWORKS=[${OS_NETWORKS}]"
fi
